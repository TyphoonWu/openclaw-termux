...
class NativeBridge {
  ...
  /// Returns device timezone ID in IANA format, e.g. "Asia/Shanghai".
  /// Falls back to "Etc/UTC" on platform errors.
  static Future<String> getDeviceTimeZoneId() async {
    try {
      final tz = await _channel.invokeMethod<String>('getDeviceTimeZoneId');
      if (tz == null || tz.isEmpty) return 'Etc/UTC';
      return tz;
    } catch (_) {
      return 'Etc/UTC';
    }
