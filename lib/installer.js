...
export function setupProotUbuntuEnvironment() {
  console.log('Setting up Ubuntu environment...');
  // switch to a domestic apt mirror depending on Ubuntu codename
  const setupScript = `
    set -e
    # detect ubuntu codename (focal, jammy, etc.) and rewrite sources.list
    if command -v lsb_release >/dev/null 2>&1; then
      codename=$(lsb_release -cs)
    else
      # fallback by reading os-release
      codename=$(grep VERSION_CODENAME /etc/os-release | cut -d= -f2 || true)
    fi
    if [ -n "$codename" ]; then
      cat <<'EOF' >/etc/apt/sources.list
# domestic mirror generated by OpenClaw installer
...
EOF
  ...
  try {
    ...
    console.error('Failed to setup Ubuntu environment:', err.message);
    return false;
  }
}
